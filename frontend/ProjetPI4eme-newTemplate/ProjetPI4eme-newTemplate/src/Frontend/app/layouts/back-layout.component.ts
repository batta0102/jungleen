      ...
      <nav class="sticky top-0 z-50 w-full border-b border-border bg-primary text-background backdrop-blur-sm px-6 py-4">
        <div class="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <div class="hidden md:flex items-center space-x-4">
            ...
            <button 
              (click)="logout()"
              class="px-4 py-2 text-sm font-medium text-background/90 hover:text-background hover:bg-primary-hover/60 rounded-lg transition-colors">
              Logout
            </button>
...
export class BackLayoutComponent {
  ...
  updateCurrentRoute(): void {
    ...
    const match = url.match(/^\/(?:back|admin)\/([^/]+)/);
