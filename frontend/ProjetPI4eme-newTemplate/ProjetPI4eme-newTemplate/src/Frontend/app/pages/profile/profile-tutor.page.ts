...
export class ProfileTutorPage {
  ...
  setRole(role: string): void {
    if (role !== 'student' && role !== 'teacher' && role !== 'tutor' && role !== 'admin') return;
    this.user.setRole(role as UserRole);
    void this.router.navigate([role === 'admin' ? '/front/profile/admin' : role === 'tutor' || role === 'teacher' ? '/front/profile/tutor' : '/front/profile/student']);
  }
}
