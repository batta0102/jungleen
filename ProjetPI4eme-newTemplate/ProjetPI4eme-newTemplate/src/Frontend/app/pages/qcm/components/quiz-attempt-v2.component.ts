...
export class QuizAttemptComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.monitorTabSwitchCount();  // ADD THIS
    this.startTimer();
  }

  /**
   * Monitor tab switch count directly from detection service
   */
  private monitorTabSwitchCount(): void {
    // Poll every 500ms to check tab switch count
    interval(500)
      .pipe(takeUntil(this.destroy$))
      .subscribe(() => {
        const count = this.quizIntegrity.getTabSwitchCount();
        
        if (count >= 2 && !this.autoSubmitTriggered) {
          console.error('🚨 [MONITOR] TAB SWITCH COUNT >= 2 DETECTED');
          console.error('🚨 [MONITOR] SETTING autoSubmitTriggered = true');
          this.autoSubmitTriggered = true;
          
          // Show alert
          console.error('🚨 [MONITOR] SHOWING CRITICAL TOAST');
          this.showToast(
            '🚨 CRITICAL: Quiz integrity violated. Submitting quiz...',
            'critical',
            3000
          );
          
          // Then force submit after brief delay
          console.error('🚨 [MONITOR] SCHEDULING SUBMIT FOR 800ms');
          setTimeout(() => {
            console.error('🚨 [MONITOR] TIMEOUT FIRED - CALLING submitAttempt DIRECTLY');
